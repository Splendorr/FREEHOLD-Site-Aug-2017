...
// let movedYet = false;
let shouldParticles = true;
let shouldRipples = true;

let target = document.querySelector('#targetFrame');
let parallaxWrapInner = document.querySelector('.parallax-wrap-inner');
let scene = document.getElementById('scene');
let grainDiv = document.getElementById('shc-grain');
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!navigator.userAgent.match(/(iPad|Firefox|Windows NT)/i)) {
    ...
    let trailerLinkDiv = document.querySelector('.trailerLink');
    let trailerPlayerDiv = document.querySelector('#myvideo');
    let trailerYT = trailerPlayerDiv.getAttribute('data-youtube-id');
  ...
  let frameLauncher = document.querySelector('.titleArt');
  if (!navigator.userAgent.match(/(Firefox)/i)) {
...
lightUpPopAndFlicker
  .add({
    targets: '.inner.lightUpFrame > .shc-frame',
    scaleY: [
      { value: 1.04, duration: 100 },
      { value: 1, duration: 1 },
    ],
    scaleX: [
      { value: 1.04, duration: 100, delay: 75 },
      { value: 1, duration: 1 },
    ],
    easing: 'easeOutQuad',
    // offset: 0,
...
let rippleDepthBase = '-833px';
let rippleDepthMax = '-2000px';
...
var animateDust = function (el, i) {
  let thisDuration = anime.random(dustDuration, (dustDuration * 1.4))
  anime({
    targets: el,
    translateZ: ['-5000px', '5000px'],
    offset: (dustDuration / maxDustElements) * i,
    opacity: [
      { value: 0, duration: thisDuration * .10 },
      { value: 1, duration: thisDuration * .15 },
      { value: 1, duration: thisDuration * .50 },
      { value: 0, duration: thisDuration * .05 },
      { value: 0, duration: thisDuration * .20 }
    ],
